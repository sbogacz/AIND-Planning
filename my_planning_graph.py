...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        for fluent in node_a1.action.effect_add:
            if fluent in node_a2.action.effect_rem:
                return True
        for fluent in node_a2.action.effect_add:
            if fluent in node_a1.action.effect_rem:
                return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        for fluent in node_a1.action.effect_rem:
            if fluent in node_a2.action.precond_pos:
                return True
        for fluent in node_a2.action.effect_rem:
            if fluent in node_a1.action.precond_pos:
                return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        for a1_parent_state in node_a1.parents:
            for a2_parent_state in node_a2.parents:
                if a1_parent_state.is_mutex(a2_parent_state):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # here we shortcircuit in reverse of the competing needs mutex,
        # since our base case (of returning True when there are no possible actions
        # necessitates that we exhaust the possible parents
        for s1_parent_state in node_s1.parents:
            for s2_parent_state in node_s2.parents:
                if not s1_parent_state.is_mutex(s2_parent_state):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal: # N.B this is bad naming
            goal_attained = False
            for cost, states in enumerate(self.s_levels):
                if goal_attained:
                    break
                for state in states:
